...
class Utils:
    ...
    def retrieve_entities(self, string):
        ...
        for entity in self.entities["entities"]:
            if self.explanation_bank_lemmatize(string) == entity:
        ...
        for word in tokenized_string:
            ...
            while check_index > head_index:
                if len(wordnet.synsets("_".join(tokenized_string[head_index:check_index]))) > 0:
                    ...
                    #depth = max(len(hyp_path) for hyp_path in wordnet.synsets("_".join(tokenized_string[head_index:check_index]))[0].hypernym_paths())
                    depth = sum(len(ss.hypernyms()) for ss in wordnet.synsets("_".join(tokenized_string[head_index:check_index])))
                    entities.append([final_entity, depth])
                    break
                check_index -= 1
